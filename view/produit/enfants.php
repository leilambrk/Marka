...
if (!$tab){
	...
	foreach ($tab as $prod){
		...
		echo '<div><div class="item" /><img class="vet" src= "'. htmlspecialchars($prod->get('photo')) . '" alt="' . htmlspecialchars($prod->get('idProduit')). '"/> </div> <div class="info"> <h2>'. htmlspecialchars($prod->get('nomProduit')) . '</h2><h3>Prix: ' . htmlspecialchars($prod->get('prix')) . '€  </h3> Taille :' . htmlspecialchars($prod->get('taille')) . '<p class="descr">' . htmlspecialchars($prod->get('description')) . '</p></div></div>';
